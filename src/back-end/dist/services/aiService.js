...
class AIService {
    static async generateReportText(weatherData, neighborhood, metrics) {
        try {
            let weatherSummary = "";
            if (weatherData && weatherData.hourly) {
                const { time, temperature_2m, precipitation, wind_speed_10m, wind_gusts_10m } = weatherData.hourly;
                weatherSummary = time.slice(-24).map((t, index) => {
                ...
                weatherSummary = weatherData.slice(-24).map((dataPoint) => `Hora: ${dataPoint.time || ""} | Temp: ${dataPoint.temperature || dataPoint.temperature_2m}C | Chuva: ${dataPoint.precipitation}mm | Vento: ${dataPoint.windSpeed || dataPoint.wind_speed_10m}km/h`).join("\n");
            }
